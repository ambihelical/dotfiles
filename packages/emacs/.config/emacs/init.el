...
(use-package display-fill-column-indicator
  ...
  ;; DejaVu Sans Mono connects, might be because it's the default font :shrug:
  (defconst me:fill-column-font "DejaVu Sans Mono-14")
...
(use-package modus-themes
  ...
  (defun me:modus-themes-after-load-theme ()
    (setq me:theme-is-dark-p (eq (car custom-enabled-themes) 'modus-vivendi))
  ...
  (use-package smart-mode-line
    ...
    )  ;; runs hooks on sml/after-setup-hook
  :defer 0)
...
(use-package eglot
  ...
  (when-let* ((clangd (seq-find #'executable-find '("clangd" "clangd-6.0"))))
    (let* ((query (if (eq window-system 'w32)
                      "--query-driver=*:\\**\\*"
                    "--query-driver=/**/*"))
           (args `((c++-mode c-mode) ,clangd ,query "--header-insertion=never" "--clang-tidy" "--background-index" "--completion-style=detailed" "-j=4")))
      (add-to-list 'eglot-server-programs args)))
