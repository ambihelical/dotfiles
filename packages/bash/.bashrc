...
if [ ${OSTYPE:0:5} == 'linux' ]; then
	...
	if [ "$ID" == "ubuntu" ]; then
		if [ -e ${XDG_CONFIG_HOME}/environment.d/ssh-agent.conf ]; then
            ...
            sockdir="$(find /tmp -type d -uid $(id -u) -name 'ssh-*' 2>/dev/null | head -n 1)"
			if [[ "$SSH_AGENT_PID" != '' && "$sockdir" != '' ]]; then
				export SSH_AUTH_SOCK=$sockdir/agent.$SSH_AGENT_PID
...
if [ "$ecma" == "1" ]; then
	...
	if [ -x /usr/bin/dircolors ]; then
		eval "`dircolors -b`"
		export LS_COLORS+=':ow=01;33:di=1;36'   # make dirs readable
	fi
